...
/// @notice Deploys AleaStrategies to Base mainnet or Base Sepolia.
///
/// Usage:
...
contract Deploy is Script {
    ...
    function run() external returns (AleaStrategies aleastategies) {
        address usdc     = _usdcForChain();
        address treasury = vm.envAddress("TREASURY");

        vm.startBroadcast();
        aleastategies = new AleaStrategies(IERC20(usdc), treasury);
        vm.stopBroadcast();

        console.log("AleaStrategies deployed at:", address(aleastategies));
